...
class GUI:
    ...
    def login_window(self):
        ...
        exit_button = Button(login_frame, text="Exit", bd=0, font=("Goudy old style", 15), bg=EXIT_BG_COLOR, fg="white", width=8, height=1, command=lambda: self.exit(login_frame))
        exit_button.place(relx=0.5, rely=0.9, anchor="center")
    ...
    def login(self, entry_login_username, entry_login_password, var_remember_me):
        ...
        if entered_username not in self.encryption.encryption_keys:
            ...
            encrypted_hashed_password = self.encryption.encrypt(hashed_password, self.encryption.encryption_keys[entered_username])
            self.client.messages = []
            ...
            if self.client.messages[1] == "success":
                while self.client.username == "":
                    pass
                self.encryption.encryption_keys[entered_username] = eval(self.client.messages[3])
                self.client.messages = []
                self.main_screen()
            else:
                self.top_levels["first_window"].iconify() # keeps the login screen
                if self.client.messages[2] == "False":
                    messagebox.showwarning("Login Failed!", "Could not find username: " +  entered_username)
                else:
    ...
    def signup_window(self):
        ...
        back_button.place(relx=0.73, rely=0.25)
        exit_button = Button(sign_up_frame, text="Exit", bd=0, font=("Goudy old style", 15), bg=EXIT_BG_COLOR, fg="white", width=8, height=1, command=lambda: self.exit(sign_up_frame))
        exit_button.place(relx=0.5, rely=0.9, anchor="center")
    ...
    def sign_up(self, entry_signup_username, entry_signup_password, var_remember_me):
        ...
        if len(entered_username) == 0:
            ...
            if len(entered_password) < 8 or not self.contains_special_characters(entered_password):
                ...
                self.encryption.encryption_keys[entered_username] = encryption_key
                self.client.messages = []
                self.client.send_message(["encryption_key", entered_username, str(encryption_key)])
                while self.client.messages == []:
                    pass # waiting till the client receives data after his encryption key sending
                self.client.messages = []
                bytes_password = entered_password.encode('utf-8')
                hashed_password = hashlib.sha256(bytes_password).digest()
                encrypted_hashed_password = self.encryption.encrypt(hashed_password, encryption_key)
                self.client.messages = []
    ...
    def main_screen(self):
        ...
        # Exit Button
        back_button = Button(main_frame, text="Exit", bd=0, font=("Goudy old style", 15), bg=EXIT_BG_COLOR, fg="white", width=8, height=1, command=lambda: self.exit(main_frame))
...
# --------------------------------------------Settings----------------
    def settings(self):
        ...
        title = Label(settings_frame, text="Settings", font=("Impact", 25, "bold"), fg="#6162FF", bg=BG_COLOR_TEXT)
        title.place(relx=0.5, rely=0.2, anchor="center")
    ...
    def get_last_score_mean(self, last_score_text, mean_title_text, status_rectangle, rectangle_id, text_id):
        ...
        while self.client.messages == []:
            pass
        last_encrypted_score_details = json.loads(self.client.messages[0])
        last_decrypted_score = self.encryption.decrypt(eval(last_encrypted_score_details[0]), eval(last_encrypted_score_details[1]), eval(last_encrypted_score_details[2]), eval(last_encrypted_score_details[3]), self.client.username)
        mean_encrypted_details = json.loads(self.client.messages[1])
        last_decrypted_mean = self.encryption.decrypt(eval(mean_encrypted_details[0]), eval(mean_encrypted_details[1]), eval(mean_encrypted_details[2]), eval(mean_encrypted_details[3]), self.client.username)
    ...
    def check_sorting(self, entry_numbers):
        if entry_numbers.get() == "":
            ...
            if self.client.messages[2] == "success":
                ...
                while self.client.messages == []:
                    pass
                encrypted_score_details = json.loads(self.client.messages[3])
                decrypted_score = self.encryption.decrypt(
                    ast.literal_eval(encrypted_score_details[0]), 
                    ast.literal_eval(encrypted_score_details[1]), 
                    ast.literal_eval(encrypted_score_details[2]), 
                    ast.literal_eval(encrypted_score_details[3]), 
    ...
    def update_chat_timer(self, subject_text, text_area, remaining_time=60):
        if remaining_time > 0:
            ...
            if self.client.new_subject == "kicking client":
                ...
                self.client.new_subject = ""
                self.top_levels["game"].after(1000, self.update_chat_timer, subject_text, text_area, 60)
